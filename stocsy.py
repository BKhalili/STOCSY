...
def stocsy(M,ppm,dist,sig,OmitNeigboringFlag):
    ...
    P=remove_Pairs(P,M,ppm,sig,dist,OmitNeigboringFlag)
...
def remove_Pairs(P,M,ppm,sig,dist,OmitNeigboringFlag):    #removes repeated and unsignificant pairs
    ...
    for i in range(P.shape[0]):
        ...
        for j in range(i+1,P.shape[0]):
            ...
            elif OmitNeigboringFlag and (ppm[P[i][0]]<ppm[P[j][0]]+dist and ppm[P[i][0]]>ppm[P[j][0]]-dist and ppm[P[i][1]]<ppm[P[j][1]]+dist and ppm[P[i][1]]>ppm[P[j][1]]-dist) or (ppm[P[i][0]]<ppm[P[j][1]]+dist and ppm[P[i][0]]>ppm[P[j][1]]-dist and ppm[P[i][1]]<ppm[P[j][0]]+dist and ppm[P[i][1]]>ppm[P[j][0]]-dist):
